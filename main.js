(function () {
  ...
  // Builds and downloads a real .pdf file directly in the browser using
  // jsPDF. This does NOT depend on the OS/browser print dialog, which is
  // unreliable on many mobile browsers and in-app WebViews (some don't
  // support "Save as PDF" at all, others render the whole page instead of
  // just the answer sheet). A directly generated PDF blob download works
  // consistently across Android, iOS, and desktop.
  function downloadLkpdPdf(caseNum) {
    var data = LKPD_CASES[caseNum];
    if (!data) return;
    ...
    if (!jsPDFCtor) {
      alert(
        "Gagal membuat PDF karena komponen pembuat PDF belum termuat. " +
        "Pastikan perangkat terhubung ke internet, lalu muat ulang halaman dan coba lagi."
      );
      return;
    }
    ...
    function addWrapped(text, fontSize, fontStyle, lineGap, gapAfter) {
    ...
    // Heading
    addWrapped(decodeBasicEntities(data.heading), 15, "bold", 18, 2);
    addWrapped(decodeBasicEntities(data.subheading), 12, "bold", 15, 14);

    // Identity
    var nama = getLkpdValue("lk-nama") || ".......................";
    var kelas = getLkpdValue("lk-kelas") || "...............";
    var absen = getLkpdValue("lk-absen") || "..........";
    addWrapped("Nama       : " + nama, 10.5, "normal", 14, 2);
    addWrapped("Kelas      : " + kelas, 10.5, "normal", 14, 2);
    addWrapped("Nomor Absen: " + absen, 10.5, "normal", 14, 12);

    addWrapped("Jawaban:", 11.5, "bold", 15, 8);

    data.steps.forEach(function (step) {
      ensureSpace(24);
      addWrapped(decodeBasicEntities(step.title), 10.5, "bold", 14, 4);
      step.items.forEach(function (item, i) {
        var letter = String.fromCharCode(97 + i);
        var value = getLkpdValue(item.key) || "..........................................";
        addWrapped(letter + ". " + item.label + " : " + value, 10, "normal", 13, 3);
      });
      y += 6;
    });

    ensureSpace(20);
    addWrapped(decodeBasicEntities(data.conclusion.label) + ":", 11.5, "bold", 15, 6);
    var conclusionValue = getLkpdValue(data.conclusion.key);
    if (conclusionValue) {
      addWrapped(conclusionValue, 10, "normal", 13, 0);
    } else {
      addWrapped("................................................................", 10, "normal", 16, 2);
      addWrapped("................................................................", 10, "normal", 16, 2);
      addWrapped("................................................................", 10, "normal", 16, 0);
    }

    ensureSpace(30);
    y += 18;
    var footer = "Dicetak dari LKPD Interaktif Barisan & Deret pada " + new Date().toLocaleDateString("id-ID");
    doc.setFont("helvetica", "italic");
    doc.setFontSize(8.5);
    doc.setTextColor(120, 120, 120);
    doc.text(footer, marginX, y);

    doc.save(buildPrintFileName(caseNum) + ".pdf");
  }

  var printBtn1 = document.getElementById("lk-print-1");
  if (printBtn1) printBtn1.addEventListener("click", function () { downloadLkpdPdf(1); });
  var printBtn2 = document.getElementById("lk-print-2");
  if (printBtn2) printBtn2.addEventListener("click", function () { downloadLkpdPdf(2); });

(function () {
  ...
  function printLkpd(caseNum) {
    ...
    // On many mobile browsers (Chrome/Safari for Android & iOS), calling
    // window.print() synchronously right after changing the DOM/class can
    // open the print dialog before the browser has actually re-rendered the
    // page with the "printing-lkpd" class applied. The result is that the
    // WHOLE page gets printed instead of just the #print-sheet content.
    // Waiting for two animation frames ensures the browser has painted the
    // updated layout before the print dialog is triggered, while still
    // being close enough to the click to count as a user-initiated action.
